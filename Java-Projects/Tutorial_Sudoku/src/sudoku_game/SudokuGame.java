...
public class SudokuGame {
	...
	public void reduceDomainForColumns() {
		...
		for (int column_index = 0; column_index < gameNumber; column_index++ ) {
			...
			for (int row_index = 0; row_index < gameNumber; row_index++) {
				...
				int vertice = 0;
				for(CSP variable : variables) {
					if ((variable.getRow() == row_index) && (variable.getColumn() == column_index)) {
						System.out.println("Variable for row/col: " + row_index + "/" + column_index + " " + variable.getVertices().toString());
						vertice = variable.getAssignment();
					}
				}
				System.out.println("gameVariable: " + vertice);
				...
				for (CSP variable : variables) {
					...
					if (variable.getColumn() == column_index) {
						...
						if (variable.getAssignment() == 0) {
							...
							variable.removeVertex(vertice);
							System.out.println("Removing variable " + vertice + " at column_index " + column_index);
							
							System.out.println("Domain: " + variable.getVertices().toString());
	...
	public void reduceDomainForRows() {
				...
				for(CSP variable : variables) {
					...
					if (variable.getRow() == row_index) {
						...
						if (variable.getAssignment() == 0) {
							...
							variable.removeVertex(vertice);
							System.out.println("Removing variable " + vertice + " at row_index " + row_index + " column_index " + column_index);
	...
	public void reduceDomainForGroups() {
		for (int column_index = 0; column_index < gameNumber; column_index++ ) {
			for (int row_index = 0; row_index < gameNumber; row_index++) {
				int vertex = 0;
				for(CSP variable : variables) {
					if ((variable.getRow() == row_index) && (variable.getColumn() == column_index)) {
						System.out.println("Variable for row/col: " + row_index + "/" + column_index + " " + variable.getVertices().toString());
						vertex = variable.getAssignment();
					}
				}
				
				System.out.println("group vertex: " + vertex + " for col " + column_index + " row " + row_index);
				if (column_index < (gameNumber/2) && row_index < (gameNumber/2)) {
					...
					System.out.println("In section 2");
					for(CSP variable : variables) {
						if ((variable.getRow() < (gameNumber/2)) && (variable.getColumn() >= (gameNumber/2))) {
							System.out.println("Domains: " + variable.getVertices().toString());
							if (variable.getAssignment() == 0) {
								variable.removeVertex(vertex);
								if (variable.getVertices().size() == 1) {
									variable.setAssignment(variable.getFirstVertex());
